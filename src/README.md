...
Reproduce the configuration used in the paper (with the ER network):
...
For Watts-Strogatz test network:
...
python experiment.py \
  ...
  network.params.n=16 \
  network.params.k=6   \
  network.params.beta=0.1  \ #default is 0.1
  experiment.max_rounds=1 \
  seed=814183
```
...
│   └── selection/          # Diversity selection utilities
│       ├── diversity.py        # Maximin selection algorithms
│       ├── graph_selection.py  # Network graph sampling
│       ├── statement_selection.py  # Medical statement selection
│       └── config_writer.py    # YAML config generation
├── scripts/                 # Executable scripts
│   ├── select_graphs.py    # CLI for graph selection
│   └── select_statements.py  # CLI for statement selection
...
## Selecting Diverse Graphs and Statements

The framework includes tools for selecting diverse network topologies and medical statements for experiments using a maximin diversity criterion.

### Selecting Network Graphs

Generate and select structurally diverse graphs:

```bash
# Select 8 diverse Erdős-Rényi graphs from a pool of 1000
python scripts/select_graphs.py \
  --generator ER \
  --n 48 \
  --pool-size 1000 \
  --k 8 \
  --er-p 0.3 \
  --output resources/seeds/graph_seeds.txt

# Select diverse Watts-Strogatz graphs
python scripts/select_graphs.py \
  --generator WS \
  --n 48 \
  --k 8 \
...
**Selection Criteria:** Graphs are selected to maximize diversity across:

- Mean shortest path length
- Global clustering coefficient
- Mean and standard deviation of node degrees
- Degree of node 0 (where the "doctor" agent is placed)

### Selecting Medical Statements

Select diverse medical statements from LLM prediction data:

```bash
# Select 30 diverse statements (15 true, 15 false)
python scripts/select_statements.py \
  --data-dir resources/data/predictions \
  --k 30 \
  --output-dir configs/statement \
  --show-diversity

# Pre-seed with specific statements
python scripts/select_statements.py \
  --k 30 \
  --init-statements \
...
**Selection Criteria:** Statements are selected using balanced maximin to ensure:

- Equal representation of true/false labels (K/2 each)
- Maximum diversity in feature space:
  - Label ground truth
  - Doctor LLM prediction accuracy
  - Other LLMs' prediction accuracy
  - Doctor-other consensus score

The script generates Hydra-compatible YAML config files in `configs/statement/` with names like `true_0.yaml`, `false_0.yaml`, etc., containing statement metadata, labels, and prediction statistics.

**Module API:** The selection functionality is also available as a Python module:

```python
from pathlib import Path
from src.selection import (
    load_statement_data,
    select_diverse_statements,
    write_statement_configs,
)

# Load prediction data
df, _ = load_statement_data(Path("resources/data/predictions"))

# Select diverse statements
features = ["label_ground_truth", "doc_accuracy", "other_accuracy", "consensus_score"]
selected_ids, df_selected, _ = select_diverse_statements(df, K=30, feature_cols=features)

# Write config files
write_statement_configs(df_selected, selected_ids)
```

See `src/selection/` for the full API including graph selection and diversity diagnostics.
...
3. **Check for completed experiments:**

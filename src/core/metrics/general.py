...
def diffusion_speed(labels: np.ndarray, categories: bool = False) -> np.ndarray:
    ...
    return diffusion_speed  # type: ignore[no-any-return]
...
def simple_influence_scores(history: Sequence[np.ndarray]) -> np.ndarray:
    ...
    return R.sum(axis=1)  # type: ignore[no-any-return]
...
def transfer_entropy_matrix(
    belief_history: list[np.ndarray], A: np.ndarray | None = None
) -> np.ndarray:
...
def information_leadership(te_matrix: np.ndarray) -> np.ndarray:
    ...
    return leadership  # type: ignore[no-any-return]
...
def graph_transfer_entropy(
    ...
    Returns:
        Tuple of (TE_matrix, total_te, mean_te)
    """
    ...
    return TE, total_te, mean_te  # type: ignore[return-value]
...
def collective_accuracy_scores(beliefs: np.ndarray, correct_label: int) -> np.ndarray:
    ...
    Raises:
        ValueError: If correct_label is not 0 or 1.
    """
    if correct_label not in [0, 1]:
        raise ValueError("correct_label must be 0 or 1")

    return 1 - np.abs(beliefs - correct_label)  # type: ignore[no-any-return]
...
def collective_accuracy_label(beliefs: np.ndarray, correct_label: int) -> np.ndarray:
    ...
    if correct_label not in [0, 1]:
        raise ValueError("correct_label must be 0 or 1")
    return (beliefs == correct_label).astype(float)  # type: ignore[no-any-return]

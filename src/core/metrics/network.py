...
def mean_shortest_path(
    ...
    """
    Compute the mean shortest path length of the graph.
    ...
    """
    D = shortest_path_lengths(A, directed=directed, unweighted=unweighted)
    n = D.shape[0]

    if directed:
        # average over all ordered pairs i != j
        mask = ~np.eye(n, dtype=bool)
        return float(D[mask].mean())
    # average over unordered pairs i < j
    tri = np.triu_indices(n, k=1)
    return float(D[tri].mean())
...
def node_wedges(A: np.ndarray) -> np.ndarray:
    ...
    return wedges  # type: ignore[no-any-return]
...
def eigenvector_centrality(
    ...
    for _ in range(max_iter):
        ...
        if np.linalg.norm(x_new - x) < tol * np.linalg.norm(x):
            return x_new  # type: ignore[no-any-return]
...
def bridging_centrality(A: np.ndarray) -> np.ndarray:
    ...
    return bet * bc  # type: ignore[no-any-return]
...
def cross_belief_fraction_discrete(beliefs: np.ndarray, A: np.ndarray) -> float:
    ...
    return float(cross.sum() / edges.sum())
...
def assortativity_categorical(labels: np.ndarray, A: np.ndarray) -> float:
    ...
    for a, b in zip(lu, lv, strict=True):
        e[a, b] += 1.0

...
class Probe:
    ...
    def __init__(self, cfg: dict, io, model_cfg: dict):
    ...
    def score(
        self, activation: np.ndarray, *args: Any, **kwargs: Any
        ...
        Returns:
            Tuple of (calibrated_label, raw_score, final_scores) where:
                - calibrated_label: Calibrated prediction (-1, 0, or 1)
                - raw_score: Raw decision function score
                - final_scores: Final score that specifies the confidence for each class (or None for Probe)
        """
        ...
        final_scores = None
        # TODO: Implement final_scores for the sAwMIL probe
        return yhat.ravel()[0], scores.ravel()[0], final_scores
...
class ZeroShotProbe:
    ...
    def __init__(self, cfg: dict, io, model_cfg: dict | None = None) -> None:
        ...
        """
        self.cfg: dict = cfg
        self.model_cfg: dict | None = model_cfg
        self.io = io
        self._name = cfg.get("name")  # probe name
        self.tokenizer = None
        self.token_ids = {}
    ...
    def _prepare_token_ids(self) -> None:
        """Prepare token IDs for answer options 1-3.
    ...
    def score(
        self, logits: np.ndarray, *args: Any, **kwargs: Any
    ) -> tuple[float, float, np.ndarray]:
        """Generate belief score from output logits.

        Extracts probabilities for answer tokens 1-3 and maps them to
        ...
        Returns:
            Tuple of (predicted_class, confidence, final_scores) where:
                - predicted_class: Predicted class (-1, 0, or 1)
                - confidence: Probability of the predicted class (float in [0.0, 1.0])
                - final_scores: Array of [P(true), P(false), P(uncertain)]
        """
        if self.tokenizer is None:
        ...
        # Token '1' → True (class 1)
        # Token '2' → False (class 0)
        # Else → Uncertain (class -1)
        p_true = probs.get("1", 0.0)
        p_false = probs.get("2", 0.0)
        p_uncertain = probs.get("3", 0.0) + probs.get("else", 0.0)
        ...
        log.debug(f"Probabilities: {probs}")
        # Return predicted class and confidence of the prediction
        final_scores = np.asarray([p_true, p_false, p_uncertain]).tolist()
        confidence = p_true  # should always return p_true
        return predicted_class, confidence, final_scores
    ...
    def _collect_probabilities(self, logits: np.ndarray) -> dict[str, float]:
        ...
        Returns:
            Dictionary mapping answer options ('1'-'3') to their probabilities.

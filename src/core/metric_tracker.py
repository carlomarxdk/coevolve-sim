...
class MetricsTracker:
    ...
    def _register_agent(self, agent, network) -> dict:
        ...
        summary = {
            "agent_id": int(agent.id),  # runtime id = node id
            "node_id": int(getattr(agent, "node_id", agent.id)),
            "catalog_id": int(getattr(agent, "catalog_id", agent.id)),
            "role": agent.role,
            ...
            "network": {
                ...
                "adjacent_agents": [int(nbr) for nbr in network.neighbors(agent.id)],
            },
        }
    ...
    def register_experiment(self, agents: dict, network) -> dict:
        ...
        for ia, agent in agents.items():
            ...
            agents_list.append(
                {
                    "id": int(ia),  # runtime id = node id
                    "node_id": int(getattr(agent, "node_id", ia)),
                    "catalog_id": int(getattr(agent, "catalog_id", ia)),
                    "model": model_name,
                    "role": role_name,
                }
            )
    ...
    def update_agent_records(
        ...
        if prev_record is not None:
            ...
            prev_complete_scores = prev_record.get("complete_scores", {}).get("curr")
        else:
            prev_belief = None
            prev_score = None
            prev_complete_scores = None

        # Get complete scores from agent if available
        complete_scores = (
            agent.current_complete_scores(t)
            if hasattr(agent, "current_complete_scores")
            else None
        )

        entry = {
            "agent_id": int(agent_id),
            "node_id": int(getattr(agent, "node_id", agent_id)),
            "catalog_id": int(getattr(agent, "catalog_id", agent_id)),
            ...
            "complete_scores": {
                "prev": prev_complete_scores,
                "curr": complete_scores,
            },
    ...
    def record_round(self, t: int, network: Any) -> dict:
        ...
        _local_agreement, mean_local_agreement, neighbor_mean_beliefs = (
    ...
    def record_run_summary(self, run_id: int, extra: dict | None = None) -> None:
